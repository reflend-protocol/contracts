 ...
 * @author Reflend
 */
contract Reflendtroller is
    ReflendtrollerV7Storage,
    ...
    ExponentialNoError
{
    ...
    event ReflendBorrowSpeedUpdated(RToken indexed lToken, uint newSpeed);

    /// @notice Emitted when a new supply-side COMP speed is calculated for a market
    event ReflendSupplySpeedUpdated(RToken indexed lToken, uint newSpeed);
    ...
    event ReflendAccruedAdjusted(address indexed user, uint oldReflendAccrued, uint newReflendAccrued);

    /// @notice Emitted when COMP receivable for a user has been updated.
    event ReflendReceivableUpdated(address indexed user, uint oldReflendReceivable, uint newReflendReceivable);
    ...
    function liquidateBorrowAllowed(
        ...
        if (isDeprecated(RToken(lTokenBorrowed))) {
    ...
    function _initializeMarket(address lToken) internal {
        ...
        if (supplyState.index == 0) {
            ...
            supplyState.index = reflendInitialIndex;
        }

        if (borrowState.index == 0) {
            // Initialize borrow state index with default value
            borrowState.index = reflendInitialIndex;
        }
    ...
    function fixBadAccruals(address[] calldata affectedUsers, uint[] calldata amounts) external {
        ...
        for (uint i = 0; i < affectedUsers.length; ++i) {
            ...
            currentAccrual = reflendAccrued[user];
            ...
            if (amountToSubtract > currentAccrual) {
                ...
                uint oldReceivable = reflendReceivable[user];
                uint newReceivable = add_(oldReceivable, accountReceivable);

                // Accounting: record the COMP debt for the user
                reflendReceivable[user] = newReceivable;

                emit ReflendReceivableUpdated(user, oldReceivable, newReceivable);
            ...
            if (amountToSubtract > 0) {
                ...
                reflendAccrued[user] = newAccrual = sub_(currentAccrual, amountToSubtract);

                emit ReflendAccruedAdjusted(user, currentAccrual, newAccrual);
            }
        }
     ...
     */
    function setReflendSpeedInternal(RToken lToken, uint supplySpeed, uint borrowSpeed) internal {
        Market storage market = markets[address(lToken)];
        require(market.isListed, "reflend market is not listed");

        if (reflendSupplySpeeds[address(lToken)] != supplySpeed) {
            ...
            reflendSupplySpeeds[address(lToken)] = supplySpeed;
            emit ReflendSupplySpeedUpdated(lToken, supplySpeed);
        }

        if (reflendBorrowSpeeds[address(lToken)] != borrowSpeed) {
            ...
            // Update speed and emit event
            reflendBorrowSpeeds[address(lToken)] = borrowSpeed;
            emit ReflendBorrowSpeedUpdated(lToken, borrowSpeed);
        }
    }
     ...
     * @dev Index is a cumulative sum of the COMP per lToken accrued.
     */
    function updateReflendSupplyIndex(address lToken) internal {
        ReflendMarketState storage supplyState = reflendSupplyState[lToken];
        uint supplySpeed = reflendSupplySpeeds[lToken];
        uint32 blockNumber = safe32(getBlockNumber(), "block number exceeds 32 bits");
    ...
    function updateReflendBorrowIndex(address lToken, Exp memory marketBorrowIndex) internal {
        ReflendMarketState storage borrowState = reflendBorrowState[lToken];
        uint borrowSpeed = reflendBorrowSpeeds[lToken];
        uint32 blockNumber = safe32(getBlockNumber(), "block number exceeds 32 bits");
    ...
    function distributeSupplierReflend(address lToken, address supplier) internal {
        ...
        // This check should be as gas efficient as possible as distributeSupplierReflend is called in many places.
        // - We really don't want to call an external contract as that's quite expensive.

        ReflendMarketState storage supplyState = reflendSupplyState[lToken];
        uint supplyIndex = supplyState.index;
        uint supplierIndex = reflendSupplierIndex[lToken][supplier];

        // Update supplier's index to the current index since we are distributing accrued COMP
        reflendSupplierIndex[lToken][supplier] = supplyIndex;

        if (supplierIndex == 0 && supplyIndex >= reflendInitialIndex) {
            // Covers the case where users supplied tokens before the market's supply state index was set.
            // Rewards the user with COMP accrued from the start of when supplier rewards were first
            // set for the market.
            supplierIndex = reflendInitialIndex;
        }

        // Calculate change in the cumulative sum of the COMP per lToken accrued
        Double memory deltaIndex = Double({mantissa: sub_(supplyIndex, supplierIndex)});

        uint supplierTokens = RToken(lToken).balanceOf(supplier);

        // Calculate COMP accrued: lTokenAmount * accruedPerRToken
        uint supplierDelta = mul_(supplierTokens, deltaIndex);

        uint supplierAccrued = add_(reflendAccrued[supplier], supplierDelta);
        reflendAccrued[supplier] = supplierAccrued;

        emit DistributedSupplierReflend(RToken(lToken), supplier, supplierDelta, supplyIndex);
    }
    ...
    function distributeBorrowerReflend(address lToken, address borrower, Exp memory marketBorrowIndex) internal {
        ...
        // This check should be as gas efficient as possible as distributeBorrowerReflend is called in many places.
        // - We really don't want to call an external contract as that's quite expensive.

        ReflendMarketState storage borrowState = reflendBorrowState[lToken];
        uint borrowIndex = borrowState.index;
        uint borrowerIndex = reflendBorrowerIndex[lToken][borrower];

        // Update borrowers's index to the current index since we are distributing accrued COMP
        reflendBorrowerIndex[lToken][borrower] = borrowIndex;

        if (borrowerIndex == 0 && borrowIndex >= reflendInitialIndex) {
            // Covers the case where users borrowed tokens before the market's borrow state index was set.
            // Rewards the user with COMP accrued from the start of when borrower rewards were first
            // set for the market.
            borrowerIndex = reflendInitialIndex;
        }
        ...
        // Calculate COMP accrued: lTokenAmount * accruedPerBorrowedUnit
        uint borrowerDelta = mul_(borrowerAmount, deltaIndex);

        uint borrowerAccrued = add_(reflendAccrued[borrower], borrowerDelta);
        reflendAccrued[borrower] = borrowerAccrued;

        emit DistributedBorrowerReflend(RToken(lToken), borrower, borrowerDelta, borrowIndex);
    }
     ...
     * @param lTokens The markets whose COMP speed to update.
     * @param supplySpeeds New supply-side COMP speed for the corresponding market.
     * @param borrowSpeeds New borrow-side COMP speed for the corresponding market.
     */
    function _setReflendSpeeds(RToken[] memory lTokens, uint[] memory supplySpeeds, uint[] memory borrowSpeeds) public {
        require(adminOrInitializing(), "only admin can set reflend speed");

        uint numTokens = lTokens.length;
        require(
            numTokens == supplySpeeds.length && numTokens == borrowSpeeds.length,
            "Reflendtroller::_setReflendSpeeds invalid input"
        );

        for (uint i = 0; i < numTokens; ++i) {
            setReflendSpeedInternal(lTokens[i], supplySpeeds[i], borrowSpeeds[i]);
        }
    }
     ...
     */
    function isDeprecated(RToken lToken) public view returns (bool) {
        return
     ...
     */
    function getReflendAddress() public view virtual returns (address) {
