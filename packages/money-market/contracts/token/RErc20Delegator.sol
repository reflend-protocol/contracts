...
/**
 * @title Reflend-Protocol's RErc20Delegator Contract
 * @notice RTokens which wrap an EIP-20 underlying and delegate to an implementation
 * @author Reflend-Protocol
 */
contract RErc20Delegator is RTokenInterface, RErc20Interface, LDelegatorInterface {
    /**
    ...
    function _setReflendtroller(ReflendtrollerInterface newReflendtroller) public override returns (uint) {
        bytes memory data = delegateToImplementation(
            abi.encodeWithSignature("_setReflendtroller(address)", newReflendtroller)
        );
        return abi.decode(data, (uint));
    }
