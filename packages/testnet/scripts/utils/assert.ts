 ...
 */
const deepEqual = (actual, expected, context?: string) => {
 ...
 */
const bnClose = (actual: BigNumber, expected: BigNumber, varianceParam: string | BigNumberish = '100000') => {
    ...
    assert.ok(
        ...
        `Number is too small to be close (Delta between actual and expected is ${evm.fromUnit(
            actualDelta
        )}, but variance was only ${evm.fromUnit(variance)}`
    )
    assert.ok(
        actual.lte(expected.add(variance)),
        `Number is too large to be close (Delta between actual and expected is ${evm.fromUnit(
            actualDelta
        )}, but variance was only ${evm.fromUnit(variance)})`
    )
}
 ...
 */
const eventEqual = (txReceiptOrEvent, expectedEvent: string, expectedArgs: Record<string, any>) => {
